...
let AuthService = class AuthService {
    ...
    async register(userData) {
        try {
            const { email, password } = userData, rest = __rest(userData, ["email", "password"]);
    ...
    async signOut(token) {
        try {
            ...
            throw new common_1.InternalServerErrorException('Sign out failed');
        }
    }
    ...
    async updatePassword(password, userId) {
        try {
            const { error } = await this.supabaseService.getAdminClient().auth.admin.updateUserById(userId, { password });
            if (error) {
                throw new common_1.UnauthorizedException(error.message);
            }
